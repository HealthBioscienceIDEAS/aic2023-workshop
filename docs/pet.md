...
3. View 4D PET data
   ...
   1. You are currently viewing individual PET frames that have not been denoised in any way.
...
counts detected, the less noisy the image will appear.
   1. Use the Volume field to advance through the PET frames from the first frame (index = 0)
...
newly created image was smoothed)
   ...
   1. Close the SPM batch editor.
   1. View the resultant smoothed images in FSL (the ones with an ‘s’ prefix in the filename).
...
included.
      1. We will use default values for Estimation Options
      1. In the Reslice Options Set Interpolation to “Trilinear” and masking to “Mask
Images”
